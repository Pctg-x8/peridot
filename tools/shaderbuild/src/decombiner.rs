...
impl<'s> Tokenizer<'s>
{
    ...
    {
        self.strip_ignores();
        if !self.strip_prefix(":") { Err(()) } else { self.glsl_represents_until_decl_end() }
    }
    /// : ... <term_char>
    pub fn glsl_type_ascription_until(&mut self, term_chars: &[char]) -> ParseResult<&'s str>
    {
        self.strip_ignores();
        if !self.strip_prefix(":") { Err(()) } else { self.glsl_represents_until(term_chars) }
    }
    ...
    fn expect_block_start(&mut self) -> ParseResult<()> { self.strip_ignores().expect_strip_prefix("{") }
    ...
    {
        if !self.strip_prefix(prefix) { Err(()) } else { Ok(()) }
    }
}
...
impl<'s> Tokenizer<'s>
{
    ...
    {
        if self.expect_block_start().is_err() { return Vec::new(); }
...
impl<'s> CombinedShader<'s>
{
    pub fn from_parsed_blocks(blocks: Vec<ToplevelBlock<'s>>) -> Self
    {
        ...
        if cs.vertex_shader_code.is_empty() { panic!("VertexShader is not defined"); }
        return cs;
    }
            ...
            for (&(set, bindings), d) in ufs.iter()
            {
                let resource_def_code = match d
                {
                    ...
                    DescriptorBoundResource::SamplerBuffer(name) =>
                        format!("uniform samplerBuffer {};\n", name),
                    DescriptorBoundResource::Sampler(dim, name) =>
                        format!("uniform sampler{}D {};\n", dim, name)
                };

                *code += &format!("layout(set = {}, binding = {}) {}", set, bindings, resource_def_code);
            }
        }
    }
        ...
        for &(binding, ref blk) in self.vertex_input.iter()
        {
            ...
            for (loc_offs, &Variable { type_str, .. }) in blk.vars.iter().enumerate()
            {
                match type_str
                {
                    ...
                    _ => warn!("Unimplemented: Cannot estimate appropriate attribute info for `{}`", type_str)
                }
            }
