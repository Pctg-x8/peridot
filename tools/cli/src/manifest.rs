...
pub fn gen_manifest(
    ...
    let manifest_content =
        std::fs::read_to_string(&manifest_template_path).expect("Failed to load cradle manifest");
    let mut manifest: CargoManifest =
        toml::from_str(&manifest_content).expect("Failed to parse cradle manifest");

    // set userlib as cradle's dependency
    manifest.dependencies.insert(
        userlib_name,
        CargoDependency::Path {
            path: userlib_path_abs,
            opts: CargoDependencyOptions {
    ...
    std::fs::OpenOptions::new()
        ...
        .write(
            toml::to_string_pretty(&manifest)
                .expect("Failed to serialize Cargo.toml")
                .as_bytes(),
        )
