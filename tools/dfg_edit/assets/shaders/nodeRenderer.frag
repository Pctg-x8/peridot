...
void main()
{
    ...
    const float dist_from_edge = min(uv_dist_from_edge.x, uv_dist_from_edge.y * (uv_scale.y / uv_scale.x));

    // AA用に追加で考慮するサイズ uv_scale座標空間で
    const float aa_ext = 1.0f;

    const float re_outer = length(max(vec2(0.0f), vec2(CornerRadius) - uv_dist_from_edge * vec2(1.0f, uv_scale.y / uv_scale.x)));
    ...
    const vec4 brightness = mix(vec4(0.3f, 0.3f, 0.3f, 1.0f), vec4(0.6f, 0.6f, 0.6f, 1.0f), uv.y);
    const vec4 face_color = mix(brightness, tint_color, 0.3f);
