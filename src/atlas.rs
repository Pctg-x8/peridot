...
impl<B: BinningAlgorithm> DynamicTextureAtlas<B> {
    ...
    pub fn to_uv(&self, p: &math::Vector2<u32>) -> math::Vector2<f32> {
        math::Vector2(p.0 as f32 / self.size.0 as f32, p.1 as f32 / self.size.1 as f32)
    }
}

/// Processes binning of TextureSlices in Dynamic Texture Atlas
pub trait BinningAlgorithm {
    fn with_texture_size(tex_size: math::Vector2<u32>) -> Self;
    fn request_rect(&mut self, size: &math::Vector2<u32>) -> Option<TextureSlice>;
}
/// Most simple binning algorithm
pub struct BookshelfBinning {
    limit: math::Vector2<u32>,
    current_base_y: u32,
    current_max_h: u32,
    current_used_x: u32
}
impl BinningAlgorithm for BookshelfBinning {
    ...
    fn request_rect(&mut self, size: &math::Vector2<u32>) -> Option<TextureSlice> {
        ...
        let offs = math::Vector2(self.current_used_x, self.current_base_y);
