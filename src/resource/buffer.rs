...
use std::rc::Rc;
use num::Integer;
use super::{Memory, AutocloseMappedMemoryRange};
...
impl Buffer {
    ...
    pub fn map(&self, range: std::ops::Range<u64>) -> br::Result<br::MappedMemoryRange> {
        self.1.map((self.2 + range.start) as _ .. (self.2 + range.end) as _)
    }
    pub unsafe fn unmap(&self) {
        self.1.unmap()
    }
    pub fn guard_map<F: FnOnce(&br::MappedMemoryRange) -> R, R>(
        &self, range: std::ops::Range<u64>, f: F
    ) -> br::Result<R> {
        Ok(f(&AutocloseMappedMemoryRange(&self.1, std::mem::ManuallyDrop::new(self.map(range)?))))
    }
}
impl std::ops::Deref for Buffer {
    type Target = br::Buffer;
    fn deref(&self) -> &br::Buffer { &self.0 }
}
...
impl BufferContent {
    ...
    pub fn vertex<T>() -> Self {
        BufferContent::Vertex(std::mem::size_of::<T>() as _, std::mem::align_of::<T>() as _)
    }
    pub fn vertices<T>(count: usize) -> Self {
        BufferContent::Vertex(std::mem::size_of::<T>() as u64 * count as u64, std::mem::align_of::<T>() as _)
    }

    pub fn index<T>()  -> Self {
        BufferContent::Index(std::mem::size_of::<T>() as _, std::mem::align_of::<T>() as _)
    }
    pub fn indices<T>(count: usize) -> Self {
        BufferContent::Index(std::mem::size_of::<T>() as u64 * count as u64, std::mem::align_of::<T>() as _)
    }

    pub fn uniform<T>() -> Self {
        BufferContent::Uniform(std::mem::size_of::<T>() as _, std::mem::align_of::<T>() as _)
    }
    pub fn uniform_dynarray<T>(count: usize) -> Self {
        BufferContent::Uniform(std::mem::size_of::<T>() as u64 * count as u64, std::mem::align_of::<T>() as _)
    }

    pub fn uniform_texel<T>() -> Self {
        BufferContent::UniformTexel(std::mem::size_of::<T>() as _, std::mem::align_of::<T>() as _)
    }
    pub fn uniform_texel_dynarray<T>(count: usize) -> Self {
        BufferContent::UniformTexel(std::mem::size_of::<T>() as u64 * count as u64, std::mem::align_of::<T>() as _)
    }

    pub fn storage<T>() -> Self {
        BufferContent::Storage(std::mem::size_of::<T>() as _, std::mem::align_of::<T>() as _)
    }
    pub fn storage_dynarray<T>(count: usize) -> Self {
        BufferContent::Storage(std::mem::size_of::<T>() as u64 * count as u64, std::mem::align_of::<T>() as _)
    }

    pub fn storage_texel<T>() -> Self {
        BufferContent::StorageTexel(std::mem::size_of::<T>() as _, std::mem::align_of::<T>() as _)
    }
    pub fn storage_texel_dynarray<T>(count: usize) -> Self {
        BufferContent::StorageTexel(std::mem::size_of::<T>() as u64 * count as u64, std::mem::align_of::<T>() as _)
    }
}
#[derive(Clone)]
pub struct BufferPrealloc<'g> {
    g: &'g crate::Graphics, usage: br::BufferUsage, offsets: Vec<u64>, total: u64, common_align: u64
}
