...
use super::{AutocloseMappedMemoryRange, Memory};
use bedrock as br;
use br::MemoryBound;
use num::Integer;
use std::rc::Rc;
...
impl Buffer {
    ...
    pub fn map(&self, range: std::ops::Range<u64>) -> br::Result<br::MappedMemoryRange> {
        self.1
            .map((self.2 + range.start) as _..(self.2 + range.end) as _)
    }
    ...
    pub fn guard_map<F: FnOnce(&br::MappedMemoryRange) -> R, R>(
        ...
        Ok(f(&AutocloseMappedMemoryRange(
            &self.1,
            std::mem::ManuallyDrop::new(self.map(range)?),
        )))
    }
}
...
impl br::VkHandle for Buffer {
    ...
    const TYPE: br::vk::VkStructureType = <br::Buffer as br::VkHandle>::TYPE;
