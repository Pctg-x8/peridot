...
pub struct Mixer {
    ...
    subprocess_pool: ThreadPool,
    parallelize: u32,
    subprocess_buffers: Vec<f32>,
    subprocess_buffers_refs: Vec<UniqueRawSliceMut<f32>>,
    master_amp: f32,
    running: bool,
    last_sample: [f32; 2],
}
impl Mixer {
    pub fn new() -> Self {
        ...
        Mixer {
            parallelize,
            subprocess_pool,
            subprocess_buffers: Vec::new(),
            subprocess_buffers_refs: Vec::new(),
            processes: Vec::new(),
            master_amp: 1.0,
            running: false,
            last_sample: [0.0; 2],
    ...
    pub fn set_sample_rate(&mut self, samples_per_sec: f32) {
        for p in &self.processes {
            p.write().set_sample_rate(samples_per_sec);
        }
    }
    ...
    fn combine_all_subprocess_buffers(&self, buf: &mut [f32]) {
        ...
        for sp in &self.subprocess_buffers_refs {
            for (b, sp) in buf
                .iter_mut()
                .zip(unsafe { sp.as_slice()[..buflen].iter() })
            {
    ...
    pub fn process(&mut self, buf: &mut [f32]) -> bool {
        ...
        let current_subprocess_buffer_len =
            self.subprocess_buffers_refs.first().map_or(0, |s| s.length);
        if buf.len() > current_subprocess_buffer_len {
            self.setup_process_frames(buf.len() as _);
        }
        self.subprocess_buffers.fill(0.0);

        let buflen = buf.len();
        self.subprocess_pool.install(|| {
            for ps in self.processes.chunks(self.parallelize as _) {
                ps.par_iter().enumerate().for_each(|(i, p)| unsafe {
                    let slice = self.subprocess_buffers_refs[i].as_slice_mut();
                    p.write().process(&mut slice[..buflen]);
                });
            }
        });
...
pub struct AudioEffectStack {
    pub effect_stack: Vec<Arc<RwLock<dyn Processor>>>,
    pub output_amp: f32,
}
...
impl Processor for AudioEffectStack {
    fn set_sample_rate(&mut self, samples_per_sec: f32) {
        for e in &self.effect_stack {
            e.write().set_sample_rate(samples_per_sec);
        }
    }
    fn process(&mut self, flattened_buffer: &mut [f32]) {
        for e in &self.effect_stack {
            e.write().process(flattened_buffer);
        }
        if self.output_amp != 1.0 {
            for b in flattened_buffer {
                *b *= self.output_amp;
            }
        }
    }
...
trait WaveSample: Copy {
    fn to_f32(self) -> f32;
}
impl WaveSample for u8 {
    fn to_f32(self) -> f32 {
...
impl WaveSample for i16 {
    fn to_f32(self) -> f32 {
        self as f32 / 0x8000 as f32
    }
}
impl WaveSample for i32 {
    fn to_f32(self) -> f32 {
        self as f32 / 0x8000_0000u32 as f32
    }
}
impl WaveSample for i64 {
    fn to_f32(self) -> f32 {
        (self as f64 / 0x8000_0000_0000_0000u64 as f64) as _
    }
}
...
impl WaveSample for Int24 {
    fn to_f32(self) -> f32 {
        self.0 as f32 / 0x80_0000 as f32
    }
}
impl From<WaveSamplesInFile> for WaveSamples {
    fn from(s: WaveSamplesInFile) -> Self {
        match s {
            WaveSamplesInFile::Mono8(v) => {
                WaveSamples::Mono(v.into_iter().map(WaveSample::to_f32).collect())
            }
            WaveSamplesInFile::Mono16(v) => {
                WaveSamples::Mono(v.into_iter().map(WaveSample::to_f32).collect())
            }
            WaveSamplesInFile::Mono24(v) => {
                WaveSamples::Mono(v.into_iter().map(WaveSample::to_f32).collect())
            }
            WaveSamplesInFile::Mono32(v) => {
                WaveSamples::Mono(v.into_iter().map(WaveSample::to_f32).collect())
            }
            WaveSamplesInFile::Mono64(v) => {
                WaveSamples::Mono(v.into_iter().map(WaveSample::to_f32).collect())
            }
            WaveSamplesInFile::MonoF64(v) => {
                WaveSamples::Mono(v.into_iter().map(|v| v as f32).collect())
            }

            WaveSamplesInFile::Stereo8(v) => WaveSamples::Stereo(
                v.into_iter()
                    .map(|v| [v[0].to_f32(), v[1].to_f32()])
                    .collect(),
            ),
            WaveSamplesInFile::Stereo16(v) => WaveSamples::Stereo(
                v.into_iter()
                    .map(|v| [v[0].to_f32(), v[1].to_f32()])
                    .collect(),
            ),
            WaveSamplesInFile::Stereo24(v) => WaveSamples::Stereo(
                v.into_iter()
                    .map(|v| [v[0].to_f32(), v[1].to_f32()])
                    .collect(),
            ),
            WaveSamplesInFile::Stereo32(v) => WaveSamples::Stereo(
                v.into_iter()
                    .map(|v| [v[0].to_f32(), v[1].to_f32()])
                    .collect(),
            ),
            WaveSamplesInFile::Stereo64(v) => WaveSamples::Stereo(
                v.into_iter()
                    .map(|v| [v[0].to_f32(), v[1].to_f32()])
                    .collect(),
            ),
...
impl<F: Read + Seek> RIFFLoader<F> {
    fn read_next_chunk_header(f: &mut F) -> IOResult<RIFFChunkHeader> {
        let mut hdr = RIFFChunkHeader {
            fourcc: 0,
            length: 0,
        };
        f.read_exact(unsafe { std::mem::transmute::<_, &mut [u8; 4 * 2]>(&mut hdr) })
            .map(move |_| hdr)
    }
    ...
    fn read_fmt(&mut self) -> IOResult<RIFFWaveFormatData> {
        ...
        self.file
            .read_exact(unsafe { std::mem::transmute::<_, &mut [u8; 16]>(&mut fmt) })?;
        return Ok(fmt);
    }
...
impl<F: InputStream> RIFFStreamingLoader<F> {
    fn read_next_chunk_header(&mut self) -> IOResult<RIFFChunkHeader> {
        ...
        self.file
            .read_exact(unsafe { std::mem::transmute::<_, &mut [u8; 4 * 2]>(&mut hdr) })
            .map(move |_| hdr)
    }
    ...
    fn seek_data(&mut self) -> IOResult<u32> {
        self.search_chunk(fourcc(b"data"))
    }
...
impl Processor for PreloadedPlayableWav {
    fn process(&mut self, flattened_buffer: &mut [f32]) {
        ...
        for (n, s) in self.samples[self.current_smp..self.current_smp + fill_count]
            .iter()
            .enumerate()
        {
    ...
    fn from_asset<Asset: super::InputStream + 'static>(asset: Asset) -> IOResult<Self> {
        ...
        let buffer_length = match loader.read_data_uncompressed(&fmt, buffered_samples.len()) {
            Ok(v) => {
                ...
                loaded.copy_into_stereo(&mut buffered_samples);
                loaded.frames()
            }
...
impl Processor for StreamingPlayableWav {
    fn process(&mut self, flattened_buffer: &mut [f32]) {
        ...
        let frames = if self.buffered_smp < self.buffer_length {
            ...
            for (n, s) in self.buffered_samples[self.buffered_smp..self.buffered_smp + frames]
                .iter()
                .enumerate()
            {
                flattened_buffer[(n << 1) + 0] = s[0];
        ...
        if self.buffered_smp >= self.buffer_length {
            self.buffer();
        }
        if self.state != PlayableAudioState::Playing {
            return;
        ...
        for (n, s) in self.buffered_samples[self.buffered_smp..self.buffered_smp + left_frames]
            .iter()
            .enumerate()
        {
...
impl StreamingPlayableWav {
    fn buffer(&mut self) {
        let loaded = match self
            .loader
            .read_data_uncompressed(&self.fmt, self.buffered_samples.len())
        {
