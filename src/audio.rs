...
struct BoxedInputStream(Box<InputStream>);
...
pub struct Mixer {
    processes: Vec<Arc<RwLock<Processor + Sync + Send>>>,
...
impl Mixer {
    ...
    pub fn add_process(&mut self, p: Arc<RwLock<Processor + Send + Sync>>) {
...
pub struct AudioEffectStack {
    pub effect_stack: Vec<Arc<RwLock<Processor>>>, pub output_amp: f32
}
...
fn fourcc(cc: &[u8; 4]) -> u32 { unsafe { *std::mem::transmute::<_, &u32>(cc) } }
unsafe fn fourcc_unsize(cc: &[u8]) -> u32 { *std::mem::transmute::<_, *const u32>(cc.as_ptr()) }
...
#[repr(C)] #[derive(Debug)] pub struct RIFFWaveFormatData {
    encoding: u16, num_channels: u8, sample_rate: u32,
    avg_bytes_per_sec: u32, block_size: u16,
    bits_per_sample: u16
}
#[repr(C)] pub enum WaveSamplesInFile {
...
impl<F: super::InputStream> From<F> for RIFFStreamingLoader<F> {
...
impl super::FromStreamingAsset for StreamingPlayableWav {
    fn from_asset<Asset: super::InputStream + 'static>(asset: Asset) -> IOResult<Self> {
        ...
        loader.seek_data()?;
        // initial buffering
        let mut buffered_samples = Vec::with_capacity(16384);
        unsafe { buffered_samples.set_len(16384); }
        ...
        let buffer_length = match loader.read_data_uncompressed(&fmt, buffered_samples.len()) {
            ...
            Err(e) => if e.kind() == ErrorKind::UnexpectedEof { 0 } else { return Err(e); }
        };
...
impl StreamingPlayableWav {
    fn buffer(&mut self) {
        let loaded = match self.loader.read_data_uncompressed(&self.fmt, self.buffered_samples.len()) {
            ...
            else { panic!("Buffering WaveSample: {:?}", e) }
        };
