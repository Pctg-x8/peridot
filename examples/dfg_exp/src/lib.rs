...
mod dfg
{
    ...
    // u16にしておくとアラインメント制約を小さく出来るので有利
    #[derive(Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
    pub enum DataTag
    {
        Float, Int, Int64, Uint, Uint64, Double,
        /// User-defined Type Identifier, Byte Size
        CustomBlob(u16, u16)
    }
    /// データの出力口
    pub struct SourceTapDescription
    {
        /// 流す型
        pub data_tag: DataTag,
        /// 値に変化がなくても毎フレームdirtyになる
        pub force_update_every_frame: bool
    }
    /// データの入力口
    pub struct SinkTapDescription
    {
        /// 受け取る型
        pub data_tag: DataTag
    }
    pub struct ElementIODescription
    {
        outputs: Cow<'static, [SourceTapDescription]>,
        inputs: Cow<'static, [SinkTapDescription]>
    }

    pub struct DFGWriteFloat { offset: usize }
    impl DFGWriteFloat
    {
        pub static IO: ElementIODescription = ElementIODescription
        {
            inputs: Cow::Borrowed(&[SinkTapDescription { data_tag: DataTag::Float }]),
            outputs: Cow::Borrowed(&[])
        };
    }
}

pub trait DataFlowGraphElement
{
    fn source_taps(&self) -> &[DataSourceTap] { &[] }
    fn sink_taps(&self) -> &[DataSinkTap] { &[] }
}

/// DFG Write to offset in local store
pub struct DFGWrite
{
    offset: usize,
    sink: [DataSinkTap; 1]
}
impl DataFlowGraphElement for DFGWrite
{
    fn sink_taps(&self) -> &[DataSinkTap] { &self.sink }
}
impl DFGWrite
{
    pub fn new(offset: usize, dt: DataTag) -> Self
    {
        DFGWrite
        {
            offset, sink: [DataSinkTap { data_tag: dt }]
        }
    }
}

/// DFG DeltaTime Source
pub struct DFGDeltaTime([DataSourceTap; 1]);
impl DataFlowGraphElement for DFGDeltaTime
{
    fn source_taps(&self) -> &[DataSourceTap] { &self.0 }
}
impl DFGDeltaTime
{
    pub fn new(force_update_every_frame: bool) -> Self
    {
        DFGDeltaTime([DataSourceTap { data_tag: DataTag::Float, force_update_every_frame }])
    }
}

use std::collections::BTreeSet;

/// Node Index, Tap Index in a Node
pub type NodeTapRef = (u32, u32);
struct SourceTapState<T>
{
    node_ref: NodeTapRef, last_value: Option<T>
}
pub struct DataFlowGraph
{
    nodes: Vec<Box<DataFlowGraphElement>>,
    /// Dirtyになって接続先に影響する可能性があるタップのインデックス
    observed_source_indices: Vec<NodeTapRef>
}
impl DataFlowGraph
{
    pub fn build(nodes: Vec<Box<DataFlowGraphElement>>, tap_connections: &[(NodeTapRef, NodeTapRef)]) -> Self
    {
        let source_node_indices = nodes.iter().enumerate().filter(|(_, n)| n.sink_taps().is_empty()).map(|(x, _)| x)
            .collect::<Vec<_>>();
        let float_source_taps = source_node_indices.iter().map(|x| (x, nodes[x].source_taps()))
            .flat_map(|(nx, taps)| taps.iter().enumerate().filter(|(_, t)| t.data_tag == DataTag::Float).map(move |(x, _)| (nx, x)))
            .map(|nref| SourceTapState::<f32> { node_ref: nref, last_value: None })
            .collect::<Vec<_>>();
        let observed_source_indices = connections.iter().map(|(s, _)| s.clone()).collect::<BTreeSet<_>>()
            .into_iter().collect();
        
        DataFlowGraph
        {
            nodes, observed_source_indices
        }
    }
    pub fn update(&self)
    {
...
impl<PL: peridot::NativeLinker> peridot::EngineEvents<PL> for Game<PL>
{
    fn init(e: &mut peridot::Engine<Self, PL>) -> Self
    {
        ...
        let (buffers, _) = FixedMemory::new(e.graphics(), bp, bp_mut, ti, &mut fm_init, &mut tfb)
