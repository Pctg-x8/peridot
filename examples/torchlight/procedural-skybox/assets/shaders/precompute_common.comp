...
float calcViewZenithCosFromParam(float uv, float h)
{
    ...
    // return uv > 0.5f ? (ch + pow(uv - 0.5, 5.0) * (1.0 - ch)) : (ch - pow(uv, 5.0) * (1.0 + ch));
    return uv > 0.5 ? (pow((uv - 0.5) * 2.0, 5.0) * (1.0 - ch) + ch) : (ch - pow(uv * 2.0, 5.0) * (1.0 + ch));
}
float calcSunZenithCosFromParam(float us)
{
   // return tan((2.0 * us - 1.0 + 0.26) * 0.75) / tan(1.26 * 0.75);
   return tan((2.0 * us - 1.0 + 0.26) * 1.1) / tan(1.26 * 1.1);
}
...
bool intersection(vec2 p, vec2 v, out vec2 pa)
{
    ...
    const float zAtmos = dot(p, p) - pow(R_EARTH + H_ATM, 2.0);
    const float zGround = dot(p, p) - pow(R_EARTH, 2.0);
