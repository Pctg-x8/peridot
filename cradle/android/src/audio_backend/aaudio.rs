...
impl Api
{
	...
	{
		unsafe { CStr::from_ptr((self.0.convert_stream_state_to_text)(state)).to_str().expect("invalid stream state text") }
	}
...
{
	fn callback(&mut self, stream_ptr: *mut self::native::AAudioStream, buf: *mut c_void, frames: usize) -> CallbackResult;
}
...
impl StreamBuilder
{
	...
	{
		extern "C" fn cb_wrapper<C: DataCallback>(stream: *mut native::AAudioStream, ctx: *mut c_void, data: *mut c_void, num_frames: i32) -> native::aaudio_data_callback_result_t
		{
...
impl Stream
{
	...
	pub fn wait_for_state_change(&mut self, input_state: StreamState, next_state: &mut StreamState, timeout_ns: Option<i64>) -> Result<()>
	{
		let r = (self.api.0.stream_wait_for_state_change)(self.ptr.as_ptr(), input_state, next_state, timeout_ns.unwrap_or(std::i64::MAX));
