...
impl Presenter {
    pub fn new(g: &peridot::Graphics, window: HWND) -> Self {
        ...
        let s = br::Surface::new_win32(g.instance(), super::module_handle(), window).expect("Failed to create Surface");
...
impl From<winapi::shared::ntdef::HANDLE> for OwnedHandle {
    fn from(h: winapi::shared::ntdef::HANDLE) -> Self { OwnedHandle(h) }
}
#[cfg(feature = "transparent")]
impl OwnedHandle {
    pub fn handle(&self) -> winapi::shared::ntdef::HANDLE { self.0 }
}
