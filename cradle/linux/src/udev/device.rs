...
impl Drop for Device {
	fn drop(&mut self) {
		unsafe { udev_device_unref(self.0.as_ptr()); }
	}
}
...
impl Device {
	pub unsafe fn from_ptr(p: *mut udev_device) -> Option<Self> { NonNull::new(p).map(Self) }
	pub fn as_ptr(&self) -> *mut udev_device { self.0.as_ptr() }

	pub fn from_syspath(udev: &super::Context, syspath: &std::ffi::CStr) -> Option<Self> {
		unsafe { NonNull::new(udev_device_new_from_syspath(udev.as_ptr(), syspath.as_ptr())).map(Self) }
	}
	...
	pub fn from_subsystem_sysname(
		udev: &super::Context, subsystem: &std::ffi::CStr, sysname: &std::ffi::CStr
	) -> Option<Self> {
		unsafe {
			NonNull::new(udev_device_new_from_subsystem_sysname(udev.as_ptr(), subsystem.as_ptr(), sysname.as_ptr()))
				.map(Self)
		}
	}
	pub fn from_device_id(udev: &super::Context, id: &std::ffi::CStr) -> Option<Self> {
		unsafe { NonNull::new(udev_device_new_from_device_id(udev.as_ptr(), id.as_ptr())).map(Self) }
	}
	...
	pub fn parent_with_subsystem_devtype(&self, subsystem: &std::ffi::CStr, devtype: &std::ffi::CStr) -> Option<Self> {
		unsafe {
			Self::from_ptr(
				udev_device_get_parent_with_subsystem_devtype(self.as_ptr(), subsystem.as_ptr(), devtype.as_ptr())
			)
		}
	}
	...
	pub fn sysattr_value(&self, sysattr: &std::ffi::CStr) -> &std::ffi::CStr {
		unsafe { std::ffi::CStr::from_ptr(udev_device_get_sysattr_value(self.as_ptr(), sysattr.as_ptr())) }
	}
	pub fn set_sysattr_value(&mut self, sysattr: &std::ffi::CStr, value: &std::ffi::CStr) -> Result<(), libc::c_int> {
		let r = unsafe { udev_device_set_sysattr_value(self.as_ptr(), sysattr.as_ptr(), value.as_ptr()) };
	...
	pub fn iter_devlinks(&self) -> super::ListIterator {
		unsafe {
			super::ListIterator::from(super::ListEntry::from_ptr(udev_device_get_devlinks_list_entry(self.as_ptr())))
		}
	}
	pub fn iter_properties(&self) -> super::ListIterator {
		unsafe {
			super::ListIterator::from(super::ListEntry::from_ptr(udev_device_get_properties_list_entry(self.as_ptr())))
		}
	}
	pub fn iter_tags(&self) -> super::ListIterator {
		unsafe {
			super::ListIterator::from(super::ListEntry::from_ptr(udev_device_get_tags_list_entry(self.as_ptr())))
		}
	}
	pub fn iter_sysattr(&self) -> super::ListIterator {
		unsafe {
			super::ListIterator::from(super::ListEntry::from_ptr(udev_device_get_sysattr_list_entry(self.as_ptr())))
		}
	}
}
