...
use async_std::io::prelude::{Write, Read, BufRead, ReadExt, SeekExt, WriteExt};
use std::io::{Result as IOResult, Error as IOError};
use async_std::io::{SeekFrom, Seek, BufReader, Cursor};
...
impl AssetEntryHeadingPair
{
    ...
    {
        writer.write(unsafe { &transmute::<_, &[u8; 8 * 2]>(self)[..] }).await.map(|_| 16)
    }
    async fn read<R: Read + Unpin>(reader: &mut R) -> IOResult<Self>
    {
        let mut sink = AssetEntryHeadingPair { byte_length: 0, relative_offset: 0 };
        reader.read_exact(unsafe { &mut transmute::<_, &mut [u8; 8 * 2]>(&mut sink)[..] }).await.map(|_| sink)
    }
}
...
impl ArchiveWrite
{
    ...
    pub async fn write<W: Write + Unpin>(&self, writer: &mut W) -> IOResult<()>
    {
        ...
        match self.0
        {
            ...
            CompressionMethod::Zlib(_) => async
            {
                ...
                Self::write_common(writer, b"pard", Some(uncompressed_bytes as u64), &encoded[..]).await
            }.await,
            CompressionMethod::Lz4(_) => async
            {
                let encoded = lz4_compression::prelude::compress(&body.into_inner());

                Self::write_common(writer, b"parz", Some(uncompressed_bytes as u64), &encoded[..]).await
            }.await,
            CompressionMethod::Zstd11(_) => async
            {
                let encoded = async_std::task::spawn_blocking(
                    || zstd::stream::encode_all(std::io::Cursor::new(body.into_inner()), 11)
                ).await?;

                Self::write_common(writer, b"par1", Some(uncompressed_bytes as u64), &encoded[..]).await
            }.await
        }
    }
    async fn write_common<W: Write + Unpin>(writer: &mut W, signature: &[u8], uncompressed_bytes: Option<u64>, body: &[u8])
        -> IOResult<()>
    {
        let checksum = crc32::checksum_ieee(body);
        writer.write_all(signature).await?;
        if let Some(ub) = uncompressed_bytes { writer.write_all(&ub.to_le_bytes()).await?; }
...
impl WhereArchive
{
    ...
    {
        if let WhereArchive::OnMemory(ref b) = self { b } else { unreachable!(); }
    }
}
enum EitherArchiveReader
{
    OnMemory(Cursor<Vec<u8>>), FromIO(BufReader<File>)
}
...
impl ArchiveRead
{
    pub async fn from_file<P: AsRef<Path>>(path: P, check_integrity: bool) -> ArchiveReadResult<Self>
    {
    ...
    async fn read_file_header<R: BufRead + Unpin>(reader: &mut R) -> ArchiveReadResult<(CompressionMethod, u32)>
    {
        ...
        let comp = match &signature
        {
            ...
            b"pard" => reader.read_exact(&mut sink_64_bits).await
                .map(|_| CompressionMethod::Zlib(u64::from_le_bytes(sink_64_bits)))?,
            b"parz" => reader.read_exact(&mut sink_64_bits).await
                .map(|_| CompressionMethod::Lz4(u64::from_le_bytes(sink_64_bits)))?,
            b"par1" => reader.read_exact(&mut sink_64_bits).await
        ...
        reader.read_exact(&mut crc32_bytes).await
            .map(move |_| (comp, u32::from_le_bytes(crc32_bytes))).map_err(From::from)
    }
    async fn read_asset_entries<R: BufRead + Unpin>(reader: &mut R) -> IOResult<HashMap<String, AssetEntryHeadingPair>>
    {
    ...
    pub async fn read_bin(&mut self, path: &str) -> IOResult<Option<Vec<u8>>>
    {
        ...
        {
            self.content.seek(SeekFrom::Start(entry_pair.byte_offset)).await?;
            let mut sink = Vec::with_capacity(entry_pair.byte_length as _);
            unsafe { sink.set_len(entry_pair.byte_length as _); }

            self.content.read_exact(&mut sink).await.map(move |_| Some(sink))
        }
        else { Ok(None) }
    }
