...
use peridot::{LogicalAssetData, FromAsset, LDRImageAsset, DecodedPixelData, PixelFormat};
use std::io::{Read, Seek, BufReader};

fn load_image<'d, D>(decoder: D) -> ImageResult<DecodedPixelData> where D: ImageDecoder<'d> {
...
impl FromAsset for PNG {
    ...
    fn from_asset<Asset: Read + Seek + 'static>(asset: Asset) -> Result<Self, ImageError> {
        image::png::PngDecoder::new(asset).and_then(load_image).map(PNG)
    }
}
impl FromAsset for TGA {
    ...
    fn from_asset<Asset: Read + Seek + 'static>(asset: Asset) -> Result<Self, ImageError> {
        image::tga::TgaDecoder::new(asset).and_then(load_image).map(TGA)
    }
}
impl FromAsset for TIFF {
    ...
    fn from_asset<Asset: Read + Seek + 'static>(asset: Asset) -> Result<Self, ImageError> {
        image::tiff::TiffDecoder::new(asset).and_then(load_image).map(TIFF)
    }
}
impl FromAsset for WebP {
    ...
    fn from_asset<Asset: Read + Seek + 'static>(asset: Asset) -> Result<Self, ImageError> {
        image::webp::WebPDecoder::new(asset).and_then(load_image).map(WebP)
    }
}
impl FromAsset for BMP {
    ...
    fn from_asset<Asset: Read + Seek + 'static>(asset: Asset) -> Result<Self, ImageError> {
        image::bmp::BmpDecoder::new(asset).and_then(load_image).map(BMP)
    }
}
