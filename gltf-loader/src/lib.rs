...
/*fn main()
{
    let mut file = File::open(std::env::args().nth(1).expect("Filename Expected"))
        .map(BufReader::new).expect("Opening File");
    let (root, mut chunks) = read_glb(&mut file).expect("Reading glTF Binary");

    println!("RootInfo: {:#?}", root);
    println!("InfoChunk: {}", info_chunk_str);

    let ro = GLTFRenderableObject::new(&root, chunks.map(|r| r.expect("Reading chunk")));
    println!("Parsed: ");
    println!("-- Rendering Groups: {:?}", ro.rendering_groups);
    println!("-- Buffers: {}", ro.buffers.len());

    println!("CommandBuffer Emulated:");
    for (nmat, (mat, v)) in ro.render_params().iter().enumerate()
    {
        ...
        for (npipe, (pipe, vs)) in v.iter().enumerate()
        {
            ...
            for mesh in vs.iter()
            {
                ...
                {
                    println!("BindIndexBuffer Buffer#{}, {:?}", ib, ibrange);
