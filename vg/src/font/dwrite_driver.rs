...
use winapi::um::d2d1::{
    ID2D1SimplifiedGeometrySink, ID2D1SimplifiedGeometrySinkVtbl, D2D1_FIGURE_BEGIN,
    D2D1_FIGURE_END, D2D1_FIGURE_END_CLOSED, D2D1_FILL_MODE, D2D1_PATH_SEGMENT,
};
...
#[repr(C)]
pub struct ComBase<VTable: 'static> {
    vtbl: &'static VTable,
    refcount: ULONG,
}
impl<VTable: 'static> ComBase<VTable> {
    ...
    unsafe extern "system" fn release(this: *mut IUnknown) -> ULONG {
        ...
        thisref.refcount -= 1;
        let last_refcount = thisref.refcount;
        if thisref.refcount == 0 {
...
pub struct PathEventReceiver {
    base: ComBase<ID2D1SimplifiedGeometrySinkVtbl>,
    paths: Vec<PathEvent>,
}
impl PathEventReceiver {
    const VTABLE: &'static ID2D1SimplifiedGeometrySinkVtbl = &ID2D1SimplifiedGeometrySinkVtbl {
        SetFillMode: Self::set_fill_mode,
        SetSegmentFlags: Self::set_segment_flags,
        BeginFigure: Self::begin_figure,
        EndFigure: Self::end_figure,
        AddLines: Self::add_lines,
        AddBeziers: Self::add_beziers,
        Close: Self::close,
        parent: IUnknownVtbl {
    ...
    pub fn new() -> *mut Self {
        Box::into_raw(Box::new(PathEventReceiver {
    ...
    unsafe extern "system" fn query_interface(
        this: *mut IUnknown,
        iid: REFIID,
        objret: *mut *mut c_void,
    ) -> HRESULT {
        *objret = null_mut();
        if iid == &ID2D1SimplifiedGeometrySink::uuidof() || iid == &IUnknown::uuidof() {
...
impl PathEventReceiver {
    unsafe extern "system" fn add_lines(
    ...
    unsafe extern "system" fn add_beziers(
        this: *mut ID2D1SimplifiedGeometrySink,
        p: *const d2::BezierSegment,
        count: u32,
    ) {
        for p in from_raw_parts(p, count as _) {
            let (p1, p2) = (
            ...
            (*(this as *mut Self))
                .paths
                .push(PathEvent::CubicTo(p1, p2, p3));
        }
    }
    unsafe extern "system" fn begin_figure(
        this: *mut ID2D1SimplifiedGeometrySink,
        p: d2::Point2F,
        _begin: D2D1_FIGURE_BEGIN,
    ) {
        let p = point2(p.x, -p.y);
        (*(this as *mut Self)).paths.push(PathEvent::MoveTo(p));
    }
    unsafe extern "system" fn end_figure(
        this: *mut ID2D1SimplifiedGeometrySink,
        end: D2D1_FIGURE_END,
    ) {
    ...
    unsafe extern "system" fn close(_this: *mut ID2D1SimplifiedGeometrySink) -> HRESULT {
        S_OK
    }
    unsafe extern "system" fn set_fill_mode(
        _this: *mut ID2D1SimplifiedGeometrySink,
        _mode: D2D1_FILL_MODE,
    ) {
        trace!("*UNIMPLEMENTED* SetFillMode with {}", _mode);
    }
    unsafe extern "system" fn set_segment_flags(
        _this: *mut ID2D1SimplifiedGeometrySink,
        _segment: D2D1_PATH_SEGMENT,
    ) {
...
unsafe impl comdrive::AsRawHandle<ID2D1SimplifiedGeometrySink> for PathEventReceiver {
    fn as_raw_handle(&self) -> *mut ID2D1SimplifiedGeometrySink {
        self as *const _ as _
    }
}

pub struct ATFRegisterScope<'a>(
    &'a comdrive::dwrite::Factory,
    comdrive::ComPtr<AssetToFontConverter>,
);
impl<'a> ATFRegisterScope<'a> {
    pub fn register(
        factory: &'a comdrive::dwrite::Factory,
        atf: comdrive::ComPtr<AssetToFontConverter>,
    ) -> std::io::Result<Self> {
        factory
            .register_font_file_loader(unsafe { &*atf.0 })
...
impl Drop for ATFRegisterScope<'_> {
    fn drop(&mut self) {
        self.0
            .unregister_font_file_loader(unsafe { &*self.1 .0 })
...
unsafe impl comdrive::AsRawHandle<IDWriteFontFileLoader> for AssetToFontConverter {
    fn as_raw_handle(&self) -> *mut IDWriteFontFileLoader {
...
impl AssetStreamBridge {
    ...
    unsafe extern "system" fn read_file_fragment(
        ...
        *fragment_start = (*(this as *mut Self))
            .asset
            .0
            .as_ptr()
            .add(file_offset as _) as *const _;

        S_OK
    }
    unsafe extern "system" fn release_file_fragment(
        _this: *mut IDWriteFontFileStream,
        _fragment_context: *mut c_void,
    ) {
    }
}
