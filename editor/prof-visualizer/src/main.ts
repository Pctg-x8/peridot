    ...
    launch() {
        this.#loadButtonView.ref.addEventListener("click", async () => {
            ...
            const lineChartContentHeight = Math.max(
                TIMELINE_CHART_TOP_MARGIN,
                // 最大値よりちょっと大きめに取る
                ...memoryChartModel.totalResident.map(x => x.y + 500.0 * MEMORY_CHART_HEIGHT_PER_BYTES),
                ...memoryChartModel.totalReserved.map(x => x.y + 500.0 * MEMORY_CHART_HEIGHT_PER_BYTES),
            );
            const timelineChartContentHeight = Math.max(...timelineChartModel.barRects.map(x => x.top + x.height));
