    ...
    pub fn run(mut self) {
            ...
            .load(std::sync::atomic::Ordering::Acquire)
        {
            ...
            if preview_composition_required {
                ...
                if let Some(cb) = preview_renderer.take_pending_update_command_buffer() {
                ...
                render_queue
                    .submit(
                        &[br::SubmitInfo::new(
                            &render_waits,
                            &render_wait_stages,
                            &[preview_renderer.command_buffer()],
...
impl<'d> WindowRenderer<'d> {
    ...
    pub fn update(
        ...
        events: &SyncEventBus,
        font_set: &FontSet,
        preview_composite: &mut preview::Composite,
        mut validate_preview: impl FnMut(&mut preview::Composite, &CustomRenderContext),
    ) -> bool {
        let composite_render_data = self.composite_renderer.update(
            self.vk_device,
            composite_tree,
    ...
    pub fn validate_render_commands(&mut self, preview_composite: &preview::Composite) {
        if !self.render_cb_invalid {
            // already valid
            return;
        }
